...
local function on_scrolling(action, is_direction)
	return function()
		...
		if not wa or wa.tiled_layout ~= "scrolling" then
			return
		end
...
function M.setup()
	...
	hl.bind(
		mainMod .. "L",
		on_scrolling("right", true),
		{ desc = "Cambiar orientación del desplazamiento[scrolling]" }
	)
